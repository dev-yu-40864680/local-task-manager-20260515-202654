...
- Repository: `local-task-manager` (private)
...
- `localStorage` 読み込み時のバリデーションは型レベルが中心です。文字列の最大長・タグ数上限・日付フォーマットの厳密検証は未実装で、手動改ざんで極端に長い文字列や過剰なタグを入れた場合、UI の表示崩れや描画パフォーマンス低下が発生する可能性があります（Security / QA レビュー指摘済み・今後の改善候補）。
- 期限関連の判定ロジック（`isDueToday` / `isOverdue` / `isDueWithin`）は実行環境のタイムゾーンに依存します。日付境界の動作はブラウザ環境に依存します。
